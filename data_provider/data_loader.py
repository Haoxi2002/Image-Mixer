...
class Dataset_Basic(Dataset):
            ...
            if draw_type == 'matplotlib':
                ...
                if draw_type == 'opencv':
                    ...
                    for j in range(self.expand):
                        ...
                        np.arange(len(data_line) * self.expand), :] = (
    ...
    def __read_data__(self):
        ...
        self.fig_data = self.data2Pixel(self.data)
        ...
        self.static = np.concatenate([
            np.amax(self.data, axis=0)[:, np.newaxis],
            np.amin(self.data, axis=0)[:, np.newaxis],
            np.median(self.data, axis=0)[:, np.newaxis],
            np.mean(self.data, axis=0)[:, np.newaxis],
            np.percentile(self.data, 25, axis=0)[:, np.newaxis],
            np.percentile(self.data, 75, axis=0)[:, np.newaxis],
            np.std(self.data, axis=0)[:, np.newaxis]
        ], axis=1)
    ...
    def __getitem__(self, index):
        ...
        static = np.concatenate([
            np.amax(self.data[s_begin:s_end], axis=0)[:, np.newaxis],
            np.amin(self.data[s_begin:s_end], axis=0)[:, np.newaxis],
            np.median(self.data[s_begin:s_end], axis=0)[:, np.newaxis],
            np.mean(self.data[s_begin:s_end], axis=0)[:, np.newaxis],
            np.percentile(self.data[s_begin:s_end], 25, axis=0)[:, np.newaxis],
            np.percentile(self.data[s_begin:s_end], 75, axis=0)[:, np.newaxis],
