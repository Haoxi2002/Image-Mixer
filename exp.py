...
class Exp_Long_Term_Forecast_VI(object):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (seq_x, seq_y, fig_x, mean, std, seq_x_mark, seq_y_mark) in enumerate(train_loader):
                iter_count += 1
                ...
                if self.model_type == 0:
                    ...
                    y_pred = self.model(fig_x, mean, std)
                else:
    ...
    def vali(self, vali_data, val_loader, criterion):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (seq_x, seq_y, fig_x, mean, std, seq_x_mark, seq_y_mark) in enumerate(val_loader):
                seq_y = seq_y.float().to(self.device)
                if self.model_type == 0:
                    fig_x = fig_x.float().to(self.device)
                    mean = mean.float().to(self.device)
    ...
    def test(self, setting, test=0):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (seq_x, seq_y, fig_x, mean, std, seq_x_mark, seq_y_mark) in enumerate(test_loader):
                seq_y = seq_y.float().to(self.device)
                if self.model_type == 0:
                    fig_x = fig_x.float().to(self.device)
                    mean = mean.float().to(self.device)
