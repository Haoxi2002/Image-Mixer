...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.static_embedding1 = nn.Sequential(
            nn.Linear(9, self.token_dim),
            nn.GELU(),
            nn.Dropout(args.dropout),
            nn.Linear(self.token_dim, self.token_dim * args.hidden_dim),
            nn.Dropout(args.dropout),
        )
        ...
        self.static_embedding2 = nn.Sequential(
            nn.Linear(9, self.token_dim),
            nn.GELU(),
            nn.Dropout(args.dropout),
            nn.Linear(self.token_dim, self.token_dim * args.hidden_dim),
        ...
        self.mlp = nn.Sequential(
            MlpBlock(self.token_dim * args.hidden_dim * 2, self.token_dim * args.hidden_dim * 4),
            nn.GELU(),
    ...
    def forward(self, x, static):
        ...
        # x = self.fc_fusion(torch.cat((x, e_static), dim=2))
